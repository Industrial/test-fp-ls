...
export function toArgs<InputArgs extends Args>(argsInputs: Array<string>,
  argsDefaults: ArgsDefaults<InputArgs>): InputArgs {
...
export type Constructor<Type> = new(...args: Array<unknown>) => Type

export function dedupe<Value>(input: Array<Value>): Array<Value> {
  const set = new Set(input)
  return [...set.values()]
}

export function ifError<Value, ValueIfError>(callback: () => Value,
  onError: ValueIfError | ((error: unknown) => ValueIfError)): Value | ValueIfError {
  try {
    return callback()
  } catch (error: unknown) {
...
if (!fileExistsSync(pathUtil.join(args.outputDirPath, args.denoTypesInputPath))) {
  const { denoTypesUrl } = args
  console.info(`Downloading Deno types from ${denoTypesUrl}...`)
  Deno.writeTextFileSync(pathUtil.join(args.outputDirPath, args.denoTypesOutputPath),
...
const denoConfig: DenoConfig = ifError(() => JSON.parse(Deno.readTextFileSync(pathUtil.join(args.projectRoot, args.denoConfigPath))),
  {})
...
if (denoConfig.importMap) {
  const denoImportMap: DenoImportMap = ifError(() => JSON.parse(Deno.readTextFileSync(pathUtil.join(args.projectRoot, denoConfig.importMap))),
    {})
...
const tsConfigInput: TsConfig = ifError(() => JSON.parse(Deno.readTextFileSync(pathUtil.join(args.projectRoot, args.tsConfigInputPath))),
  {})
...
Deno.writeTextFileSync(pathUtil.join(args.projectRoot, args.tsConfigOutputPath),
